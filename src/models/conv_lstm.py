...
class ConvLSTM1D_Attention2(nn.Module):
    ...
    def forward(self, x):
        ...
        # Reshape output to match the input size of fully connected layer
        output = output.view(batch_size, -1)
        # print(f'output.shape after  {output.shape}')
        # print(f'input.shape before  {x.shape}')
        selected_array = x[:, -1:, :] 
        # print(f'selected_array.shape before  {selected_array.shape}')

        residual = selected_array.repeat(1, 500, 1) 
        # print(f'reshaped.shape before  {residual.shape}')       # Replicate it 5 times along the specified dimension (dimension 1 in this case)
        
        residual = torch.flatten(residual, 1)


        # print(f'reshaped.shape after  {residual.shape}')
